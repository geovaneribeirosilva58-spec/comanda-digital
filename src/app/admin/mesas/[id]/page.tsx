...
export default async function ComandaPage({ params }: { params: { id: string } }) {
  ...
  const orderTotalGross = order?.order_items?.reduce((acc: number, item: any) => acc + (item.status !== 'cancelado' && item.unit_price > 0 ? item.unit_price * item.quantity : 0), 0) || 0
  const partialPayments = order?.order_items?.reduce((acc: number, item: any) => acc + (item.status !== 'cancelado' && item.unit_price < 0 ? Math.abs(item.unit_price * item.quantity) : 0), 0) || 0
        ...
        <div className="p-4 bg-slate-900/50 border-b border-slate-800 flex justify-between items-center">
          ...
          {partialPayments > 0 ? (
            <div className="flex flex-col items-end text-sm">
              <span className="text-slate-400">Total: R$ {orderTotalGross.toFixed(2)}</span>
              <span className="text-emerald-500">Pago: R$ {partialPayments.toFixed(2)}</span>
              <span className="font-bold text-amber-500 text-lg mt-1">Falta: R$ {orderTotalRemaining.toFixed(2)}</span>
            </div>
          ) : (
            <span className="font-bold text-amber-500 text-lg">R$ {orderTotalGross.toFixed(2)}</span>
          )}
        </div>
        
        {order && partialPayments > 0 && (
          <div className="p-3 bg-slate-950 flex justify-end">
            <PartialPaymentButton orderId={order.id} tableId={tableId} remainingTotal={orderTotalRemaining} />
          </div>
        )}
        {order && partialPayments === 0 && (
          <div className="p-3 bg-slate-950 flex justify-end">
            ...
            (() => {
              ...
              return groupedItems.map((item: any) => (
                <div key={item.groupedIds.join('-')} className="p-4 flex justify-between items-start">
                  <div>
                    {item.unit_price < 0 ? (
                      <div className="font-medium text-emerald-500 flex items-center uppercase tracking-wide">
                        {item.note || 'Pagamento Parcial'}
                      </div>
                    ) : (
                      <>
                    ...
                    <div className="mt-2">
                      {item.unit_price > 0 && item.status === 'pendente' && (
                      ...
                      {item.unit_price > 0 && item.status === 'entregue' && <span className="inline-flex items-center text-xs font-medium text-emerald-500 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded"><CheckCircle className="w-3 h-3 mr-1"/> Entregue</span>}
                  ...
                  <div className={item.unit_price < 0 ? "font-semibold text-emerald-500" : "font-semibold text-amber-500"}>
